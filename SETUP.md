...
If PowerShell prevents the activation script from running:
...
Your API key should remain only in your local:
...
The backend then runs the MAGI debate engine and streams events back to the frontend as they happen.
...
The Debate Log displays events generated by the MAGI backend during analysis.
...
The log is scrollable, allowing users to review earlier events instead of only seeing the most recent entries.
...
1. The file exists at `backend/.env`.
2. It contains:
